...
AS
BEGIN
	--ALTER TABLE tblAttendanceSummary ADD PercentProbation FLOAT(53)
	...
	SELECT *, CAST(NULL AS DATETIME) TerminateDate, CAST(NULL AS DATETIME) HireDate, CAST(NULL AS DATETIME) ProbationEndDate, CAST(NULL AS INT) isForeign
	...
	INSERT INTO #tblAttendanceSummary (Year, Month, EmployeeID, HireDate, TerminateDate, ProbationEndDate, PeriodID, SalaryHistoryID, FromDate, ToDate, isForeign, PercentProbation)
	SELECT @Year, @Month, sh.EmployeeID, HireDate, TerminateDate, ProbationEndDate, 0, sh.SalaryHistoryID, CASE
			WHEN sh.DATE < @FromDate
				THEN @FromDate
			ELSE sh.DATE
			END, @ToDate, te.isForeign, te.PercentProbation
	FROM #EmployeeList te
	...
	BEGIN
		...
		WHERE (sh.ProbationEndDate BETWEEN @FromDate AND @ToDate OR sh.ProbationEndDate > @ToDate) AND sh.HireDate <> sh.ProbationEndDate

		UPDATE #tblAttendanceSummary
		SET PercentProbation = NULL
		FROM #tblAttendanceSummary sh
		WHERE sh.ProbationEndDate IS NULL OR sh.ProbationEndDate < @FromDate

		--het thu viec trong thang nay
		UPDATE #tblAttendanceSummary
		SET FromDate = DATEADD(day, 1, ProbationEndDate)
		FROM #tblAttendanceSummary sh
		WHERE ISNULL(PercentProbation, 0) > 0 AND ProbationEndDate BETWEEN @FromDate AND @ToDate

		INSERT INTO #tblAttendanceSummary (Year, Month, EmployeeID, HireDate, TerminateDate, PeriodID, SalaryHistoryID, FromDate, ToDate, PercentProbation)
		SELECT @Year, @Month, sh.EmployeeID, sh.HireDate, sh.TerminateDate, 1, sh.SalaryHistoryID, CASE
				WHEN @FromDate < HireDate
					THEN HireDate
				ELSE @FromDate
				END, ProbationEndDate, 100
		FROM #tblAttendanceSummary sh
	...
	FROM #tblAttendanceSummary att
	WHERE ISNULL(att.IsForeign, 0) = 0 AND WorkingDays_Total > RegularWorkdays * 8
	...
	INSERT INTO tblAttendanceSummary (Year, Month, EmployeeID, PeriodID, SalaryHistoryID, FromDate, ToDate, WorkingHrs_Total, WorkingDays_Total, PaidLeaveDays_Total, UnpaidLeaveDays, PaidLeaveHrs_Total, UnpaidLeaveHrs, STD_WorkingDays, Std_Hour_PerDays, RegularWorkdays, PercentProbation)
	SELECT Year, Month, EmployeeID, ISNULL(PeriodID, 0), SalaryHistoryID, FromDate, ToDate, WorkingHrs_Total, WorkingDays_Total, PaidLeaveDays_Total, UnpaidLeaveDays, PaidLeaveHrs_Total, UnpaidLeaveHrs, STD_WorkingDays, Std_Hour_PerDays, RegularWorkdays, PercentProbation
	FROM #tblAttendanceSummary;
