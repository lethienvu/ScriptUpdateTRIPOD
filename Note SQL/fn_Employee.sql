...
BEGIN
	...
	DELETE s
	FROM #SummaryData s
	LEFT JOIN #tblOTSummary o ON s.EmployeeID = o.EmployeeID AND s.PeriodID = o.PeriodID
	...
	SELECT @Query += N'UPDATE s SET [' + ISNULL(ColumnDisplayName, '') + '] = ISNULL(w.ApprovedHours, 0),
                        ...
                        FROM #SummaryData s
                        LEFT JOIN #tblOTSummary w ON s.EmployeeID = w.EmployeeID AND s.PeriodID = w.PeriodID
    ...
    INNER JOIN #SummaryData d ON s.EmployeeID = d.EmployeeID AND s.PeriodID = d.PeriodID
    ...
    INNER JOIN tblAttendanceSummary s ON s.EmployeeID = d.EmployeeID AND s.PeriodID = d.PeriodID
    WHERE s.Year = ' + CAST(@Year AS VARCHAR(4)) + ' AND s.Month = ' + CAST(@Month AS VARCHAR(2)) + ';'
	...
	IF ISNULL(@isView, '') = 0
		...
		FROM tblAttendanceSummary a
		INNER JOIN #SummaryData d ON a.EmployeeID = d.EmployeeID AND a.PeriodID = d.PeriodID
