...
BEGIN
	...
	SELECT @Query += N'IF COL_LENGTH(''tblAttendanceSummary'',''' + LeaveCode + N''') is null ALTER TABLE tblAttendanceSummary ADD [' + LeaveCode + N'] FLOAT;'
	FROM tblLeaveType
	WHERE IsVisible = 1

	EXEC (@Query)

	-- Tạo bảng tạm chỉ lấy các cột cần thiết
	SELECT Year, Month, EmployeeID, PeriodID, SalaryHistoryID, FromDate, ToDate, WorkingHrs_Total, WorkingDays_Total, PaidLeaveDays_Total, UnpaidLeaveDays, PaidLeaveHrs_Total, UnpaidLeaveHrs, STD_WorkingDays, Std_Hour_PerDays, RegularWorkdays, CAST(NULL AS DATETIME) TerminateDate, CAST(NULL AS DATETIME) HireDate, CAST(NULL AS FLOAT) PercentProbation, CAST(NULL AS DATETIME) ProbationEndDate, CAST(NULL AS INT) isForeign
...
SELECT * FROM #EmployeeList return
	-- Loại bỏ nhân viên đã khóa tháng
	DELETE e
	FROM #EmployeeList e
	...
	WHERE NOT EXISTS (
			...
			WHERE cs.SalaryHistoryID = sh.SalaryHistoryID
	...
	SET PeriodID = 0
	WHERE ToDate < @ToDate OR EmployeeID IN (
			SELECT EmployeeID
			FROM #tblAttendanceSummary s
			GROUP BY EmployeeID
			HAVING COUNT(1) = 1
	...
	INTO #tblOTList
	FROM tblOTList ot
	WHERE ot.OTDate BETWEEN @FromDate AND @ToDate AND ot.Approved = 1 AND ApprovedHours <> 0 AND EXISTS (
			SELECT 1
			FROM #EmployeeList te
			WHERE ot.EmployeeID = te.EmployeeID
			)
	...
	-- Tạo chỉ mục tạm thời cho bảng tạm lớn
	CREATE INDEX idx_Emp_Period ON #tblAttendanceSummary (EmployeeID, PeriodID)
